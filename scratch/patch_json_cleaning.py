...
PHYS_NOTEBOOK_PATH = r"d:\mduy\source\repos\EXACT\src\llm\tuning\train_physics_kaggle.ipynb"

# ----------------------------------------------------
# 1. Patch train_folc_kaggle.ipynb
# ----------------------------------------------------
print(f"Loading {FOLC_NOTEBOOK_PATH}...")
with open(FOLC_NOTEBOOK_PATH, "r", encoding="utf-8") as f:
    nb_folc = json.load(f)

# Find Cell 6
cell_folc = None
for cell in nb_folc["cells"]:
    if cell["cell_type"] == "code" and cell.get("id") == "0ece368f":
        cell_folc = cell
        break

if cell_folc:
    print("Patching train_folc_kaggle.ipynb Cell 6...")
    new_source = [
        ...
        "            \n",
        "    # 2. Extract first valid JSON array or object\n",
        "    first_brace = response.find(\"{\")\n",
        "    first_bracket = response.find(\"[\")\n",
        "    \n",
        "    if first_brace != -1 and (first_bracket == -1 or first_brace < first_bracket):\n",
        "        # Extract outer JSON object (try fully closed first, fallback to open-ended)\n",
        ...
        "        # Extract outer JSON array (try fully closed first, fallback to open-ended)\n",
        ...
        "            outputs = model.generate(\n",
        "                **inputs,\n",
        "                max_new_tokens=1024, # Increased to prevent truncation\n",
        "                do_sample=False,\n",
        "                pad_token_id=tokenizer.pad_token_id,\n",
        "                eos_token_id=tokenizer.eos_token_id\n",
        "            )\n",
    ...
    cell_folc["source"] = new_source
    print("Writing train_folc_kaggle.ipynb back...")
    with open(FOLC_NOTEBOOK_PATH, "w", encoding="utf-8") as f:
        json.dump(nb_folc, f, indent=1, ensure_ascii=False)
    print("Successfully patched train_folc_kaggle.ipynb!")
else:
    print("Warning: Cell 6 not found in train_folc_kaggle.ipynb.")


# ----------------------------------------------------
# 2. Patch train_physics_kaggle.ipynb
# ----------------------------------------------------
print(f"\nLoading {PHYS_NOTEBOOK_PATH}...")
with open(PHYS_NOTEBOOK_PATH, "r", encoding="utf-8") as f:
    nb_phys = json.load(f)

cell_phys = None
for cell in nb_phys["cells"]:
    if cell["cell_type"] == "code" and cell.get("id") == "training_sft":
        cell_phys = cell
        break

if cell_phys:
    print("Patching train_physics_kaggle.ipynb Cell 6...")
    new_source = [
        "# 6. Configure SFTTrainer and start training\n",
        "import os\n",
        "import glob\n",
    ...
    cell_phys["source"] = new_source
    print("Writing train_physics_kaggle.ipynb back...")
    with open(PHYS_NOTEBOOK_PATH, "w", encoding="utf-8") as f:
        json.dump(nb_phys, f, indent=1, ensure_ascii=False)
    print("Successfully patched train_physics_kaggle.ipynb!")
else:
    print("Warning: Cell 6 not found in train_physics_kaggle.ipynb.")
