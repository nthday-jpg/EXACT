...
def analyze_mismatches():
    ...
    for item in failed_cases:
        ...
        for g, p in zip(gt, pred):
            ...
            # E.g. GT: "ForAll(x, (Park(x) OR Movies(x)))" vs Pred: "ForAll(x, (Student(x) -> (GoToPark(x) OR GoToMovies(x))))"
            if "Student(" in p_str and "Student(" not in g_str:
                has_domain_rest = True
            elif "Person(" in p_str and "Person(" not in g_str:
                has_domain_rest = True

            # 4. Logical structure differences (AND/OR, different implication directions, parentheses structure)
            # Normalize whitespace/parentheses to see if it's just syntax
            g_norm = g_str.replace(" ", "").replace("(", "").replace(")", "")
            p_norm = p_str.replace(" ", "").replace("(", "").replace(")", "")
            # Check if operators themselves differ
            ...
            # If they have same/similar operators but different names
    ...
    # Let's print examples for each category to be super sure!
    for cat, items in categories.items():
        if not items:
            continue
        print(f"\n--- Examples of {cat} (Total: {len(items)}) ---")
        for eid, desc in items[:3]:
            # find item
            for item in failed_cases:
