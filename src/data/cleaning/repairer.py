...
class LogicalDatasetRepairer:
    ...
    def _call_llm_json(
        ...
        try:
            response_text = self.llm_client.generate_text(
                user_prompt, system_prompt=system_prompt, max_new_tokens=2048
            )
    ...
    def repair_sample(
        ...
        print("    [Standard Repair] Triggering turn-based feedback repair...")
        messages = [
            {"role": "system", "content": STANDARD_REPAIR_SYSTEM_PROMPT},
            {
                "role": "user",
                "content": STANDARD_REPAIR_USER_TEMPLATE.format(
        ...
        for turn in range(1, max_retries + 1):
            ...
            repaired_sample = self._call_llm_json(
                STANDARD_REPAIR_SYSTEM_PROMPT, user_prompt
            )
            ...
            for f in rep_formulas:
                f_clean = re.sub(
                    r"\b(ForAll|Exists)\(\s*\[\s*([a-zA-Z_]\w*)\s*\]\s*,", r"\1(\2,", f
                )
                match_multi = re.search(
                    r"\b(ForAll|Exists)\(\s*\[\s*([a-zA-Z_]\w*)\s*,\s*([a-zA-Z_]\w*)\s*\]\s*,",
                    f_clean,
                )
                if match_multi:
            ...
            messages.append(
                {"role": "assistant", "content": json.dumps(repaired_sample)}
            )
        ...
        repaired_sample = self._call_llm_json(
            DEEP_REPAIR_SYSTEM_PROMPT, deep_user_prompt
        )
        ...
        if repaired_sample and isinstance(repaired_sample, dict):
            ...
            if is_valid:
                print(
                    "      - [SUCCESS] Deep Repair succeeded in resolving type mismatches!"
                )
                repaired_sample.pop("validation_error", None)
                return True, repaired_sample
        ...
        diag_user = DIAGNOSTIC_USER_TEMPLATE.format(
        ...
        with open(diag_file, "a", encoding="utf-8") as df:
            if not diag_exists:
                df.write(
                    "# 🔍 Automated Logic Repair Diagnostics & Strategies Report\n\n"
                )
                df.write(
                    "This report is generated dynamically by the LogicalDatasetPipeline when a sample fails auto-repair.\n\n"
                )
            ...
            if diag_report:
                df.write(
                    f"- **Root Cause Analysis**: {diag_report.get('root_cause_analysis', 'N/A')}\n"
                )
                df.write(
                    f"- **Recommended Steps**: {diag_report.get('repair_steps', 'N/A')}\n"
                )
                df.write(
                    f"- **Suggested Repair FOL**:\n  ```json\n  {json.dumps(diag_report.get('repaired_premises_fol', []), indent=2)}\n  ```\n\n"
                )
            else:
                df.write(
        ...
        bad_result["validation_error"] = (
            f"Failed to repair. Technical diagnostics appended to {diag_file.name}."
        )
