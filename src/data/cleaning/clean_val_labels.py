...
def main():
    ...
    for idx, item in enumerate(val_samples, 1):
        ...
        if (
            ...
            and explanation.strip().lower() != "none"
        ):
            print(
                f"[{idx}/{len(val_samples)}] Sample {example_id} already verified: '{original_ans}'"
            )
        ...
        while not success and retry_count < 5:
            try:
                ...
                if response.status_code == 200:
                    ...
                    resp_text = resp_json["candidates"][0]["content"]["parts"][0][
                        "text"
                    ]
                    ...
                    if corrected_ans:
                        ...
                        if (
                            corrected_ans.strip().lower()
                            != str(original_ans).strip().lower()
                        ):
                            print(
                                f"  -> CORRECTED: '{original_ans}' -> '{corrected_ans}'"
                            )
                    ...
                    print(
                        f"  -> [Rate Limit 429] Exceeded quota. Sleeping for {sleep_time + 2.0:.2f} seconds before retrying..."
                    )
    ...
    print("\n" + "=" * 60)
    print(
        f"Completed verification. Found {len(corrections)} successfully processed samples."
    )
    ...
    if corrections:
        ...
        print(
            f"Successfully updated {updated_augmented_count} samples in {augmented_path.name}"
        )
        ...
        if original_path.exists():
            ...
            print(
                f"Successfully updated {updated_original_count} samples in {original_path.name}"
            )
