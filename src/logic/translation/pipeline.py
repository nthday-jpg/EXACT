...
class NLToFOLPipeline:
    ...
    def generate_glossary(self, nl_list: list[str]) -> dict | None:
        ...
        user_prompt = GLOSSARY_USER_PROMPT_TEMPLATE.format(
        ...
        try:
            ...
            if isinstance(glossary, dict) and (
                "predicates" in glossary or "constants" in glossary
            ):
                return glossary
        except Exception:
            pass
        ...
        for formula in formulas:
            ...
            for op, val in comp_matches:
                ...
                if (
                    val
                    and not val.isdigit()
                    and val not in {"x", "y", "z", "w", "u", "v", "a", "b", "c"}
                ):
                    constants.add(val)
    ...
    def translate_list(
        self, nl_list: list[str], max_new_tokens: int = None, glossary_str: str = None
    ) -> list[str]:
        ...
        if glossary_str:
            if is_finetuned:
                ...
                system_prompt = TRANSLATE_SYSTEM_PROMPT_GLOSSARY_TEMPLATE.format(
                    glossary_str=glossary_str
                )

            user_prompt = TRANSLATE_USER_PROMPT_TEMPLATE.format(
                nl_content=nl_content.strip(), num_premises=len(nl_list)
            )
            ...
            try:
                response_content = self._generate_text(
                    system_prompt, user_prompt, max_new_tokens=_max_tokens
                )
                all_extracted_fol = extract_fol_formulas(response_content)
                all_extracted_fol = [
                    normalize_logic_fol_entry(fol) for fol in all_extracted_fol
                ]
                all_extracted_fol = self._validate_and_repair(all_extracted_fol)
                return unify_fol_predicates(all_extracted_fol)
            except Exception as e:
                print(
                    f"Warning: Glossary-constrained translation failed ({str(e)}). Falling back to standard pipeline."
                )
        ...
        if is_finetuned:
            ...
            try:
                ...
                print(
                    f"Warning: Direct translation failed ({str(e)}). Falling back to standard pipeline."
                )
        ...
        try:
            ...
            user_prompt = COMBINED_GLOSSARY_AND_TRANSLATION_USER_PROMPT_TEMPLATE.format(
                nl_content=nl_content.strip()
            )
            ...
            print(
                f"Warning: Combined glossary and translation failed ({str(e)}). Falling back to two-stage translation."
            )
    ...
    def _repair_fol(self, formula: str, error: str) -> str:
        ...
        user_prompt = REPAIR_FOL_USER_PROMPT_TEMPLATE.format(
    ...
    def _validate_and_repair(
        self, formulas: list[str], max_retries: int = 2
    ) -> list[str]:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=min(len(formulas) + 1, 8)
        ) as executor:
            # Map single validation and repair over formulas
            results = list(
                executor.map(
                    lambda f: self._validate_and_repair_single(f, max_retries), formulas
                )
            )
        return results
    ...
    def translate_premises_and_conclusion(
        ...
        if len(all_extracted_fol) == expected_count:
            ...
            print(
                f"Warning: Combined translation length mismatch ({len(all_extracted_fol)} vs {expected_count}). Falling back to sequential glossary-aligned translation."
            )
            premises_fol = []
            ...
            try:
                res_conclusion = self.translate_list(
                    [conclusion_for_translation], glossary_str=glossary_str
                )
            except TypeError:
