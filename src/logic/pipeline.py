...
from src.logic.z3_verifier import verify_with_z3

class LogicalReasoningPipeline:
    """
    End-to-End Logical Reasoning Pipeline that translates natural language premises
    and a conclusion into First-Order Logic (FOL) formulas, verifies entailment
    mathematically using Z3 (via contradiction proof), and generates step-by-step
    natural language reasoning explaining the Z3 result (unsat core or counterexample).
    ...
    def verify_with_z3(self, premises_fol: list[str], conclusion_fol: str) -> dict:
        """Parses FOL formulas, tracks them in Z3 solver, and checks for entailment via contradiction."""
        return verify_with_z3(premises_fol, conclusion_fol)
    ...
    def run_pipeline(self, premises_nl: list[str], conclusion_nl: str) -> dict:
        """Executes the entire end-to-end logical translation, verification, and reasoning pipeline."""
        premises_fol, conclusion_fol = self.translate_premises_and_conclusion(premises_nl, conclusion_nl)
        verification = self.verify_with_z3(premises_fol, conclusion_fol)
