...
def extract_premises_used(verification: Dict[str, Any], filt_premises_nl: List[str], original_premises: List[str]) -> List[int]:
    """
...
async def predict(request: PredictRequest):
    ...
    if request.type == "type1":
        if not logic_pipeline:
            raise HTTPException(status_code=500, detail="Logic pipeline not initialized.")
            
        try:
            # 1. Adapt options into query if query doesn't list A, B, C, D and options list is non-standard
            query_formatted = request.query
            is_yes_no = all(opt.lower() in ("yes", "no", "uncertain") for opt in request.options)
            has_abcd_in_query = any(f"\n{letter}." in request.query for letter in ("A", "B", "C", "D"))
            ...
            answer = map_pipeline_answer_to_options(result.get("answer"), request.options)
            
            # Extracted filtered premises list (returned by our modified logic pipeline)
            filt_premises_nl = result.get("premises_nl", [])
            premises_used = extract_premises_used(result.get("verification", {}), filt_premises_nl, request.premises)
            ...
            if answer not in ("Uncertain", "Unknown", "No conclusion can be drawn", "") and not premises_used:
                premises_used = []
            ...
            if result.get("cot"):
                reasoning = ReasoningObject(
                    type="fol" if result.get("verification", {}).get("result") == z3.unsat else "cot",
                    steps=result["cot"]
                )
            elif result.get("reasoning"):
                reasoning = ReasoningObject(
                    type="cot",
                    steps=[line.strip() for line in result["reasoning"].splitlines() if line.strip()]
                )
                
            elapsed = time.time() - start_time
            print(f"[{request.query_id}] Type 1 processed in {elapsed:.2f}s. Answer: {answer}")
        ...
        try:
            ...
            try:
                ...
                if physics_analysis:
                    explanation_parts.append("Physics Analysis:\n" + "\n".join(f"- {step}" for step in physics_analysis))
                if algebraic_reasoning:
                    explanation_parts.append("Algebraic Reasoning:\n" + "\n".join(f"- {step}" for step in algebraic_reasoning))
                
                explanation = "\n\n".join(explanation_parts) or f"Calculated answer: {ans_str} with unit {unit_str}."
                reasoning_steps = physics_analysis + algebraic_reasoning
            except Exception:
                explanation = result.raw_response or result.error or "Executed python code to compute answer."
                reasoning_steps = [line.strip() for line in explanation.splitlines() if line.strip()]
            ...
            elapsed = time.time() - start_time
            print(f"[{request.query_id}] Type 2 processed in {elapsed:.2f}s. Answer: {ans_str} {unit_str}")
